...
We begin by establishing a few basic facts about $\sigma$:
...
*Proof.* The idea is that, since $a$ and $b$ are coprime, each divisor of $ab$ can be expressed as the product of a divisor of $a$ and of a divisor of $b$ in exactly one way.
...
**Note:** Together, Claims 2 and 3 allow us to write down a complete characterisation of $\sigma(a)$ depending on the prime factorisation of $a$: if $a = p_1 ^{\alpha_1} \cdots p_k^{\alpha_k}$, then 
...
is an integer, so $2^{k+1} - 1$ divides $2^{k+1}x$. But $2^{k+1} - 1$ and $2^{k+1}$ are coprime, so $2^{k+1} - 1$ must in fact divide $x$. Hence $y$ is an integer. 
...
Again from the definiton of $y$, we have $x = y (2^{k + 1} - 1) = 2^{k+1} y - y = \sigma(x) - y$, which leads us to $\sigma(x) = x + y$. Suppose for a contradiction that $y > 1$. Then $\sigma(x) \ge 1 + y + x > x + y$, which gives the contradiction we wanted. It follows that $y = 1$, and so $\sigma(x) = x + 1$, therefore $x$ is prime by Claim 1.
...
In general, $\sigma$ is a very interesting function to play around with, and has surprising links to many ideas in number theory. One can even reformulate the Riemann Hypothesis in terms of $\sigma$: the Riemann Hypothesis is equivalent to the statement that
