...
*Proof.* We need to check that the inverse of $f$ is continuous. This is equivalent to checking that, for every open $U \subseteq X$, the set $f(U)$ is open. This, in turn,
         is equivalent to checking that for every closed $U \subseteq X$, the set $f(U)$ is closed. We will do that.
         Let $U \subseteq X$ be closed. Since $U$ is a closed subset of a compact space, $U$ is compact. Then, since $f$ is continuous, the set $f(U)$ is compact in $Y$.
         Since $f(U)$ is a compact subset of a Hausdorff space, $f(U)$ is closed. This completes the proof. $\blacksquare$
...
Since $f$ is continuous, and constant on fibers, the universal property of quotients asserts the existence of a unique continuous map $\hat{p}$ which makes the following diagram
commute:

<br>
<div style="text-align: center"><img src="/images/quotients2.png" alt="Commutative diagram" width="50%"/></div>
<br>
